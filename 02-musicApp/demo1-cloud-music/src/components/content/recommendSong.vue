<template>
  <div class="resong">
    ...
    <div class="item" v-for="(item, index) in recommendSongs" :key="index" @mouseleave="MouseMove = false"
      @mousemove="MouseMove =  index">
      <!-- 鼠标经过显示 -->
      <div v-show="MouseMove ===  index">
        <i @click="loadMusic(item.id)" class="iconfont icon-bofang play_icon"></i>
        <div :class="{ songs_top_bottom: MouseMove === index }" class="title text_hidden" :style="{zIndex: 99}">
          {{ item.copywriter }}
        </div>
      </div>
      <!-- 鼠标离开隐藏 -->
      <div :class="{songs_top_bottom: MouseMove !== index}" class="title playCount text_hidden"
      ...
      <!-- 图片 -->
      <img @click="itemClick(item)" v-lazy="item.picUrl || item.coverImgUrl">
      <!-- 名字 -->
