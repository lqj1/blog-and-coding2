<template>
  <div>
    ...
    <div class="mv">
      <div class="mv_item" v-for="(item, index) in mv" :key="item.id" @mouseleave="MvMouseMove = false"
        @MvMouseMove="MvMouseMove =  index" @click="mvDetail(item)">
        <img v-lazy="item.picUrl" alt="" />
        <!-- 播放数量 -->
        <span class="playCount" v-show="MvMouseMove !==  index" :class="{ songs_top_bottom: MvMouseMove === index }">
        ...
        <!-- 遮罩文字 -->
        <span class="write" v-show="MvMouseMove ===  index" :class="{ songs_top_bottom: MvMouseMove === index }">
          {{ item.copywriter }}
        </span>
        <!-- 歌名 -->
